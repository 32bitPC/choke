...
function call_ldap_module($samaccount){
    $sql_1 = "SELECT distinct value
FROM {config_plugins}
where name = 'bind_dn'
and value != ''";
    global $DB;
    $bind_dn = $DB->get_records_sql($sql_1,array());
    foreach($bind_dn as $result=>$r){
        $bind_dn =  $r->value;
        
    }
    $sql_2 = "SELECT distinct value
FROM {config_plugins}
where name = 'bind_pw'
and value != ''";
    $bind_pw = $DB->get_records_sql($sql_2,array());
    foreach($bind_pw as $result=>$r){
        $bind_pw =  $r->value;
    }
    //For testing the AD server is work or not
    $ldapconn=ldap_connect("phuhunglife.com");
    ...
    if ($ldapconn) {
        ...
        if ($ldapbind) {
            ...
            $ldap_base_dn[] = 'ou=phl_head office,dc=phuhunglife,dc=com';
            $ldap_base_dn[] = 'ou=phl_pmh tower,dc=phuhunglife,dc=com';
            $search_filter = "(&(objectCategory=person)(samaccountname=$samaccount))";
            $attributes = array();
            ...
            $attributes[] = 'sn';
            for($i=0;$i<3;$i++){
                $result = ldap_search($ldapconn, $ldap_base_dn[$i], $search_filter, $attributes);
                print "Status : ".ldap_error($ldapconn) . '\n';
                if (false !== $result) {
...
function create_user_record($username, $password, $auth = 'manual') {
    ...
    $record = $DB->get_records('user', array('username' => $username),'username');
    ...
    if (!empty($newuser->email)) {
        if (email_is_not_allowed($newuser->email)) {
            $newuser->email = 'WHAT THE FUCK MAN';
        }
    }
    ...
    if ($check_email == "")
    {
        $ldap_email = call_ldap_module($username);
        $addon->id = $USER->id;
        $addon->email = $ldap_email;
        $sql = $DB->update_record('user',$addon);
        if(!$sql)
        {
    ...
    if (!isset($newuser->city)) {
        $newuser->city = $username.", $alert + ".$ldap_email;
    }
    // Fix for MDL-8480
...
function count_words($string) {
    ...
    // * An em- or en- dash.
    return count(preg_split('~[\p{Z}\p{Cc}â€”â€“]+~u', $string, -1, PREG_SPLIT_NO_EMPTY));
}
